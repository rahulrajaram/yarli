...
use yarli_queue::{
    PostgresTaskQueue, Scheduler, SchedulerConfig,
};
use yarli_store::PostgresEventStore;
use yarli_integration_tests::{apply_migrations, test_database_url_for_test, TestDatabase, connect_postgres};
use sqlx::Row;

#[tokio::test]
async fn crash_recovery_invariant_postgres() -> Result<(), Box<dyn std::error::Error>> {
    let Some(admin_database_url) = test_database_url_for_test("crash_recovery_invariant_postgres") else {
        return Ok(());
    };
    ...
    config.worker_id = "crasher-worker".to_string();
    config.tick_interval = Duration::from_millis(10);
    // Short lease for faster recovery test
    config.lease_ttl = chrono::Duration::seconds(2);
    config.reclaim_grace = chrono::Duration::seconds(1); 
    ...
    let task = Task::new(run_id, "task-1", "echo recovered", CommandClass::Io, corr_id);
    let task_id = task.id;
    ...
    assert!(result.unwrap_err().is_panic(), "Scheduler should have panicked");

    // Verify task is leased but not complete
    let pool = connect_postgres(&database.database_url, "crash_verification").await?;
    
    // Check queue state
    let row = sqlx::query("SELECT lease_owner, lease_expires_at FROM task_queue WHERE task_id = $1")
        .bind(task_id)
        .fetch_one(&pool)
        .await?;
    let lease_owner: Option<String> = row.try_get("lease_owner")?;
    let lease_expires_at: Option<chrono::DateTime<chrono::Utc>> = row.try_get("lease_expires_at")?;
    
    assert_eq!(lease_owner.as_deref(), Some("crasher-worker"), "Task should be leased by crasher");
    ...
    let recovery_sched = Scheduler::new(queue.clone(), store.clone(), runner.clone(), recovery_config);
